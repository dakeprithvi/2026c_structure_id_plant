...
comparing md5 sums before and after pdflatex is run), then pdflatex will be
...
the pdf output is set to 1 second before the tex input so that the resulting
...
def getfilecheck(ext=None, directory=False, exists=True, toAbs=False):
    """
    Returns a function to check whether inputs are valid files/directories.
    """
    def filecheck(s):
        s = str(s)
        if toAbs:
...
class CitationIndexFile(IndexFile):
    ...
    def _do_processing(self, stdout=None, backup=True):
        """Cleans the citation index (if it exists) and then runs makeindex."""
        console.status("Cleaning citation index <{}>", self.displayname)
        cleaner = CitationIndexCleaner()
        if Path(self.abspath).is_file():
            cleaner.clean(self.abspath)
            if backup:
                bak = self.abspath + ".clean"
                console.status("Saving <{}> as <{}>", self.displayname,
                               os.path.relpath(bak))
