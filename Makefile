...
ARTICLE_SRC := supplementary.tex paper.tex report.tex coverletter.tex highlights.tex
TALK_SRC :=
POSTER_SRC := 
...
INKSCAPE_SRC := Block_VAc_measurement.svg Block_VAc_2.svg structured_learning_stoic.svg \
				Economics.svg abstract.svg
...
OCT_MAT_SRC := 
MATLAB_MAT_SRC := 
PY_PICKLE_SRC := VAc_collect_param_fit_nn.py VAc_collect_param_fit_bbox_nn.py \
				 VAc_meas.py VAc_perturb_plant.py \
				 VAc_param_fit_paresto.py VAc_perturb_bbox_plant.py VAc_hist.py \
				 VAc_collect_param_fit_lr.py VAc_perturb_plant_2d.py VAc_perturb_bbox_plant_2d.py
...
PY_PLOT := VAc_plot_perturb_plant.py  VAc_plot_train_val_param_nn.py  \
		   VAc_plot_param_fit_paresto.py VAc_plot_perturb_bbox_plant.py \
		   VAc_plot_train_val_param_bbox_nn.py VAc_plot_hist.py \
		   VAc_plot_lr.py
...
## arXiv-ready bundle derived from paper.zip (see rule near the zip distribution).
PAPER_ARXIV_ZIP := paper_arxiv.zip
...
CLEAN_FILES := \
    ...
    $(MARKDOWN_DOCX) $(MAKEFILE_JSON) $(PAPER_DIST_ZIP) $(PAPER_ARXIV_ZIP) \
...
DIR_NAME := $(notdir $(TOP_DIR))

ifeq (podpush,$(firstword $(MAKECMDGOALS)))
  PODPUSH_FILES := $(wordlist 2,$(words $(MAKECMDGOALS)),$(MAKECMDGOALS))
  ifneq ($(PODPUSH_FILES),)
    $(foreach f,$(PODPUSH_FILES),$(eval $(f):;@:))
  endif
endif

podpush:
ifeq ($(PODPUSH_FILES),)
	@echo "Starting full sync to pod..."
	rsync -avz --exclude='.git' ~/projects/$(DIR_NAME)/ pdake@pod-login1.cnsi.ucsb.edu:~/projects/$(DIR_NAME)
else
	@echo "Pushing files to pod: $(PODPUSH_FILES)"
	printf '%s\n' $(PODPUSH_FILES) | rsync -avz --files-from=- ~/projects/$(DIR_NAME)/ pdake@pod-login1.cnsi.ucsb.edu:~/projects/$(DIR_NAME)
endif
.PHONY: podpush

podssh:
	ssh pdake@pod-login1.cnsi.ucsb.edu
.PHONY: podssh


ifeq (podpull,$(firstword $(MAKECMDGOALS)))
  PODPULL_SUBDIR := $(wordlist 2,$(words $(MAKECMDGOALS)),$(MAKECMDGOALS))
  ifeq ($(PODPULL_SUBDIR),)
    PODPULL_SUBDIR := cluster_pickle_param
  else
    $(eval $(PODPULL_SUBDIR):;@:)
  endif
endif
...
podpull:
	echo "Starting sync of $(PODPULL_SUBDIR) from pod..."
	rsync -avz pdake@pod-login1.cnsi.ucsb.edu:~/projects/$(DIR_NAME)/$(PODPULL_SUBDIR)/ ~/projects/$(DIR_NAME)/$(PODPULL_SUBDIR)

BRAID_HOST := braid2.cnsi.ucsb.edu

ifeq (braidpush,$(firstword $(MAKECMDGOALS)))
  BRAIDPUSH_FILES := $(wordlist 2,$(words $(MAKECMDGOALS)),$(MAKECMDGOALS))
  ifneq ($(BRAIDPUSH_FILES),)
    $(foreach f,$(BRAIDPUSH_FILES),$(eval $(f):;@:))
  endif
endif

braidpush:
ifeq ($(BRAIDPUSH_FILES),)
	@echo "Starting full sync to braid..."
	rsync -avz --exclude='.git' ~/projects/$(DIR_NAME)/ pdake@$(BRAID_HOST):~/projects/$(DIR_NAME)
else
	@echo "Pushing files to braid: $(BRAIDPUSH_FILES)"
	printf '%s\n' $(BRAIDPUSH_FILES) | rsync -avz --files-from=- ~/projects/$(DIR_NAME)/ pdake@$(BRAID_HOST):~/projects/$(DIR_NAME)
endif
.PHONY: braidpush

braidssh:
	ssh pdake@$(BRAID_HOST)
.PHONY: braidssh

ifeq (braidpull,$(firstword $(MAKECMDGOALS)))
  BRAIDPULL_SUBDIR := $(wordlist 2,$(words $(MAKECMDGOALS)),$(MAKECMDGOALS))
  ifeq ($(BRAIDPULL_SUBDIR),)
    BRAIDPULL_SUBDIR := cluster_pickle_param_nn
  else
    $(eval $(BRAIDPULL_SUBDIR):;@:)
  endif
endif

braidpull:
	@echo "Starting sync of $(BRAIDPULL_SUBDIR) from braid..."
	rsync -avz pdake@$(BRAID_HOST):~/projects/$(DIR_NAME)/$(BRAIDPULL_SUBDIR)/ ~/projects/$(DIR_NAME)/$(BRAIDPULL_SUBDIR)
.PHONY: braidpull
...
## xbib files. Use bibexport, then repair the entry types that bibexport does
## not recognize (e.g. @software or @dataset), which it writes as a bare "@{"
## that BibTeX skips.
$(PAPER_BUILD_XBIB) : %.xbib : %.pdf
	@echo Making $@
	@cd $(BUILD_DIR) && bibexport -o $(@F) $(basename $(@F))
	@mv $@.bib $@
	@./$(SCRIPT_DIR)/fixxbib.py --aux $(basename $@).aux $@
...
## arXiv-ready bundle: start from paper.zip, then add the figure used by
## supplementary.tex (paper.zip carries only the paper's own dependencies), so
## arXiv compiles the supplementary as a second document and appends it to the
## paper. The script also renames the Inkscape "PDF + LaTeX" figures, since
## arXiv deletes any X.pdf sharing its base name with an X.tex, and replaces
## the \externaldocument reference by the supplementary's labels, since arXiv
## deletes .aux files as well.
## build/supplementary.pdf is a prerequisite only to guarantee its .aux exists.
$(PAPER_ARXIV_ZIP) : paper.zip $(BUILD_DIR)/supplementary.pdf \
                     $(FIGURES_DIR)/paper/VAc_plot_lr.pdf
	@echo Making $@
	@./$(SCRIPT_DIR)/makearxivdist.py --output $@ \
	    --add $(FIGURES_DIR)/paper/VAc_plot_lr.pdf \
	    --xr-labels $(BUILD_DIR)/supplementary.aux paper.zip
...
## Publish a clean, source-only snapshot of the master branch to the github remote.
## Strips backups/ and pickles; never touches the working tree or origin.
github :
	@bash publish-github.sh
.PHONY: github

## Sync the backups/ pickles to a '<repo>_backups' dir next to the repo on the origin server.
## Host and path are derived automatically from the 'origin' remote URL.
backups_rsync :
	@url=$$(git remote get-url origin); \
	host=$$(echo "$$url" | sed -E 's|^ssh://([^/]+)/.*|\1|'); \
	path=$$(echo "$$url" | sed -E 's|^ssh://[^/]+/+|/|'); \
	dir=$${path}_backups; \
	echo "Syncing backups/ -> $$host:$$dir/"; \
	rsync -avh --info=progress2 backups/ "$$host:$$dir/"
.PHONY: backups_rsync

## Pull the backups/ pickles FROM the '<repo>_backups' dir on the origin server.
## Run this after a fresh 'git clone' to fetch the shared pickles.
backups_pull :
	@url=$$(git remote get-url origin); \
	host=$$(echo "$$url" | sed -E 's|^ssh://([^/]+)/.*|\1|'); \
	path=$$(echo "$$url" | sed -E 's|^ssh://[^/]+/+|/|'); \
	dir=$${path}_backups; \
	echo "Pulling $$host:$$dir/ -> backups/"; \
	rsync -avh --info=progress2 "$$host:$$dir/" backups/
.PHONY: backups_pull
