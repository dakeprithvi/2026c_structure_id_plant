...
plants = ['test', 'cmeas_bbox', 'cmeas_noise_bbox']
...
meas_key = {'test': 'test', 'cmeas_bbox': 'test', 'cmeas_noise_bbox': 'test'}
mc_count = 10
data_plant = {}
...
merged = load_pickle_optional('VAc_collect_param_fit_bbox_nn.pickle') or {}
data_param = load_pickle_optional('VAc_param_fit_paresto.pickle')
data_base = load_pickle_optional('VAc_meas.pickle')['test']
BASE_NN = 'VAc_param_fit_bbox_param_noise'
ID_CASE = 'test'

# friendly plant name -> (base, noise_tag) coordinates in the collected bbox
# pickle, whose nested layout is
#   merged[base][case][mc_iter][noise_tag].
nn_source = {
    'cmeas_bbox':       (BASE_NN, 'nonoise'),
    'cmeas_noise_bbox': (BASE_NN, 'noise')
}
...
def nn_data(plant, mc_iter):
    ...
    produced)."""
    base, noise_tag = nn_source[plant]
    try:
        return merged[base][ID_CASE][mc_iter][noise_tag]
    except (KeyError, TypeError):
        return None
...
# just iterates over these pre-generated guesses. These match the guesses in
# VAc_perturb_plant.py exactly (same base, seeds and perturbation), so the bbox
# and greybox studies share the same multistart points.
...
for mc_idx, seed in enumerate(seeds):                 # ── OUTER MC: guess index
    ...
    for plant, id_mcs in plant_id_mcs.items():        # ── each case
        ...
        for id_mc in id_mcs:                          # ── case's own init MC
            ...
            elif plant == 'cmeas_bbox':
                data = nn_data(plant, id_mc)
                Plant.mean = data['mean_y']
                Plant.std = data['std_y']
                Plant.mean_u = data['mean_u']
                Plant.std_u = data['std_u']
                Plant.nn_bbox = data['nvec'][0]
                param = 'bbox' # dummy param
                Plant.mean_dcdt = data['mean_dcdt']
                Plant.std_dcdt = data['std_dcdt']
            elif plant == 'cmeas_noise_bbox':
                data = nn_data(plant, id_mc)
                Plant.mean = data['mean_y']
                Plant.std = data['std_y']
                Plant.mean_u = data['mean_u']
            ...
            bounds['NO'] = [(data_meas['opti_conc']['no'], data_meas['opti_conc']['no'])]
            bounds['NEe'] = [(data_meas['opti_conc']['ne'], data_meas['opti_conc']['ne'])]
            bounds['NA'] = [(data_meas['opti_conc']['na'], data_meas['opti_conc']['na'])]
            ...
            # 'test' is the analytic-kinetics baseline (no NN): it must go through
            # the unified optimize(), which dispatches on self.param='test'.
            # optimize_cbox is NN-only and would fail with nn_bbox unset.
            if plant == 'test':
