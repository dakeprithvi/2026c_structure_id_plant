...
def getincludedfiles(texbase, texmfhome=False, bib=False):
    ...
    if texmfhome:
        ...
        texmfhome = None
    files = []
    seen = set() ## Files are stored by base name, so keep base names unique.
    for file in sorted(rawfiles):
        (_, fileext) = os.path.splitext(file)
        if fileext in keepexts and os.path.basename(file) not in seen:
            filedir = os.path.dirname(file)
            if filedir == "" or filedir.startswith("."):
                files.append(os.path.normpath(os.path.join(texdir, file)))
            elif texmfhome is not None and file.startswith(texmfhome):
                files.append(file)
            else:
                continue
            seen.add(os.path.basename(file))
