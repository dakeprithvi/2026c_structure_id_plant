...
#
# ---------------------------------------------------------------------------
# LR / line-search due-diligence variant of VAc_param_fit_nn_param_noise.py.
#
# The pretraining (cheat) stage is UNCHANGED and still runs every invocation.
# What differs is the main ODE-fit loop, now parametrized by two extra
# command-line args so the same file covers the 8-case matrix:
...
#
# Purpose: check whether the parity fit degrades below its post-pretraining
# quality purely because AdamW takes non-line-searched steps. To that end we
# also log the rate-parity RMSE every epoch and its post-pretraining baseline
# (parity_init), so each pickle is directly plottable. NOT for the paper.
#
# Usage: python VAc_param_fit_nn_param_noise_lr.py <case_type> <mc_iter> [cheat] [noise] [lr] [mode]
# 8-case matrix (one process per config):
#   for lr in 1e-3 1e-2 5e-2 1e-1; do
#     for mode in adamw hybrid; do
#       python VAc_param_fit_nn_param_noise_lr.py test 0 1 0 $lr $mode
#     done
#   done
# ---------------------------------------------------------------------------
...
# --- MC configuration ---
# Usage: python ..._lr.py <case_type> <mc_iter> [cheat] [noise] [lr] [mode]
# cheat/noise default to the module flags above; pass 0/1 to override.
# lr defaults to 5e-2, mode defaults to 'hybrid' (the original behaviour).
...
lr   = float(sys.argv[5]) if len(sys.argv) > 5 else 5e-2
mode = sys.argv[6] if len(sys.argv) > 6 else 'hybrid'
...
# each (case, mc_iter, cheat, lr, mode) run writes its own pickle here so
# parallel jobs never collide.
os.makedirs('pickle', exist_ok=True)

# log file per iteration / config
log_file_base = os.path.basename(__file__).replace('.py', '')
log_handle = open(f'log/{log_file_base}_{case_type}_{mc_iter}_{cheat_tag}_{noise_tag}_{lr_tag}_{mode}.txt', 'w')
...
# validation batched exactly like training, from the SAME self-consistent
# regenerated `val` (not the raw measurements): equal segments of the training
# segment length (val_index // num_of_batch), each solved from its own IC.  This
# covers the COMPLETE validation set at the training horizon (no single long
# free-rollout, so no train/val horizon mismatch).
...
if cheat:
    # ------------------------------------------------------------------
    # Stage 0 -- direct reaction-rate pretraining (the "cheat"). UNCHANGED
    # from VAc_param_fit_nn_param_noise.py -- this variant only differs in the
    # main ODE-fit loop below.
    # ------------------------------------------------------------------
...
# --- Rate-parity metric (the direct parity-plot quantity) --------------------
# NN rates vs true rates over the (clean) measurement trajectory, normalized
# per rate so r1 and r2 contribute comparably. parity_init is the post-
# pretraining baseline every epoch is compared against.
temp_meas = params['T'][:val_index]
temp_val  = params['T'][val_index:]

def parity_rmse(nv):
    rt1, rt2, rn1, rn2 = save_rxn(nv, clean_meas, temp_meas, params)
    e1 = (rn1 - rt1) / (jnp.std(rt1) + 1e-30)
    e2 = (rn2 - rt2) / (jnp.std(rt2) + 1e-30)
    return float(jnp.sqrt(jnp.mean(e1 ** 2) + jnp.mean(e2 ** 2)))

parity_init = parity_rmse(nvec)
parity_container = []
print(f'parity_init (post-pretraining rate-parity RMSE): {parity_init:.6f}', flush=True)
...
counter = 0
strategy = [5, 5] # number of minibatches
...
for step in range(epochs):
    ...
    # per-segment solve as training (each segment from its own measured IC) ->
    # same horizon as the training loss, on the same standardized scale.
    # loss_fn uses weights=1 and weight_q=0.
    val_sol = batch_solve(val_c0_batch, val_t_batch, nvec, params)
    val_pred_norm = (val_sol.ys - params['mean'][:7]) / params['std'][:7]
    val_norm      = (val_batch  - params['mean'][:7]) / params['std'][:7]
    mse_val = float(jnp.mean(jnp.square((val_norm - val_pred_norm) * params['weights'])))
    lossv_container.append(mse_val)
    parity_container.append(parity_rmse(nvec))   # track parity every epoch
    if mse_val < best_loss:
        ...
        counter += 1
    print(f'best_val_loss: {best_loss:.6f}, parity_rmse: {parity_container[-1]:.6f}')
    if counter > patience:
...
data['parity_init']   = parity_init
data['parity']        = parity_container
data['steps_strategy'] = steps_strategy   # the AdamW->LBFGS switch epoch (hybrid)
data['nvec']          = nvec
data['q_pred']        = Qpred
data['q_meas']        = qmeas
...
PickleTool.save(data, f'pickle/{log_file_base}_{case_type}_{mc_iter}_{cheat_tag}_{noise_tag}_{lr_tag}_{mode}.pickle')

print(f'\n Training time (hrs): {training_time / 60}', flush=True)
print(f'\n RMSE meas: {rmse_meas}', flush=True)
print(f'\n RMSE val: {rmse_val}', flush=True)
print(f'\n parity_init: {parity_init:.6f}, parity_final: {parity_container[-1]:.6f}', flush=True)
